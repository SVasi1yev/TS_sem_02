...
void ServerImpl::Start(uint16_t port, uint32_t n_acceptors, uint32_t n_workers, struct timeval tv) {
    ...
    for (int i = 0; i < n_workers; i++) {
        ...
        _workers.back().Start(_data_epoll_fd, tv);
    }
    ...
    while (run.load()) {
        ...
        for (int i = 0; i < nmod; i++) {
            ...
            if (current_event.data.fd == _event_fd) {
                ...
                run.store(false);
                if (epoll_ctl(acceptor_epoll, EPOLL_CTL_DEL, _server_socket, &event)) {
                    throw std::runtime_error("Failed to delete file descriptor to epoll");
                }
                if (epoll_ctl(acceptor_epoll, EPOLL_CTL_DEL, _event_fd, &event2)) {
                    throw std::runtime_error("Failed to delete file descriptor to epoll");
                }
                continue;
            }
            ...
            for (;;) {
                ...
                Connection *pc = new Connection(infd, this);
                if (pc == nullptr) {
