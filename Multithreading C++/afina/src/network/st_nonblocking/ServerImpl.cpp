...
inline void ServerImpl::DeleteConnection(Connection *pc) {
    close(pc->_socket);
    delete pc;
    conns.erase(pc);
}
...
// See Server.h
void ServerImpl::Start(uint16_t port, uint32_t n_acceptors, uint32_t n_workers, struct timeval tv) {
    ...
    _work_thread = std::thread(&ServerImpl::OnRun, this, tv);
}
...
void ServerImpl::OnRun(struct timeval tv) {
    ...
    while (run.load() || ((nmod > 0) && !conns.empty())) {
        nmod = epoll_wait(epoll_descr, &mod_list[0], mod_list.size(), timeout);
        ...
        for (int i = 0; i < nmod; i++) {
            ...
            if (current_event.data.fd == _event_fd) {
                ...
                if (epoll_ctl(epoll_descr, EPOLL_CTL_DEL, _server_socket, &event)) {
                    throw std::runtime_error("Failed to delete file descriptor to epoll");
                }
        ...
        for (auto pc : conns)
        {
            if ((pc->command_to_execute == nullptr) && (pc->responses.empty()))
            {
                pc->OnClose();
                if (epoll_ctl(epoll_descr, EPOLL_CTL_DEL, pc->_socket, &pc->_event)) {
                    _logger->error("Failed to delete connection from epoll");
...
void ServerImpl::OnNewConnection(int epoll_descr) {
    for (;;) {
        ...
        Connection *pc = new (std::nothrow) Connection(infd, this);
        if (pc == nullptr) {
